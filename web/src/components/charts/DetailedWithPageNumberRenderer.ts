...
import { FamDetails, IndiDetails } from "topola";
import { formatDateOrRange } from "topola";
 ...
 * and death.
 */
export class DetailedWithPageNumberRenderer
  ...
  implements Renderer
{
  ...
  private getFamDetails(fam: FamDetails): DetailsLine[] {
    ...
    if ((fam as OroksegFamDetails).getPageNumber()) {
      detailsList.push({
        symbol: "",
        text: `Page: ${(fam as OroksegFamDetails).getPageNumber()}`,
      });
    }
    return detailsList;
  }
  ...
  render(enterIn: TreeNodeSelection, updateIn: TreeNodeSelection): void {
    const enter = enterIn.append("g").attr("class", "detailed");
    const update = updateIn.select("g") as Selection<
      SVGGElement,
    ...
    const indiUpdate = (
      enter.merge(update).selectAll("g.indi") as Selection<
        BaseType,
        OffsetIndi,
        SVGGElement,
        HierarchyPointNode<TreeNode>
...
.generation-1 .detailed rect.bygeneration, .generation11 .detailed rect.bygeneration {
  ...
  private transition<B extends BaseType, T>(
    selection: Selection<B, T, BaseType, {}>,
  ) {
  ...
  private renderIndi(
    enterIn: Selection<BaseType, OffsetIndi, BaseType, {}>,
  ...
  private renderFamily(enterIn: TreeNodeSelection, update: TreeNodeSelection) {
    let enter;
